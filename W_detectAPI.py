...
class DetectAPI:
    def __init__(self, weights, imgsz=640):
        ...
        # Load model 加载模型
        # self.model = DetectMultiBackend(weights, self.device, data="data/coco128.yaml", dnn=False)
    ...
    def detect(self, source):
        ...
        # DataLoader 加载数据
        # 直接从 source 加载数据
        dataset = MyLoadImages(source)  # dataset将图像整理为files，这里是一张图像，图像尺寸没有发生变化，dataset的img_size属性，为640
        ...
        for im, im0s in dataset:
            with dt[0]:
                ...
                # im = im.half() if self.model.fp16 else im.float()  # uint8 to fp16/32 这里有个问题就是，即便self.model.half()，model已经变为半精度，但是self.model.fp16仍然为false，所以im仍然为float32
